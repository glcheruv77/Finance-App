...
async function analyzeReceipt(file) {
  ...
  try {
    ...
    if (total) {
      ...
      rewardForScanning(); // ✅ Earn points for scanning
    } else {
...
async function analyzePDF(file) {
  ...
  try {
    ...
    if (allText.trim().length > 30) {
      ...
      if (total) {
        addTotalToBudget(total, "expense", "PDF Total");
        rewardForScanning(); // ✅ Earn points for scanning
      }
      return;
    }
    ...
    if (total) {
      addTotalToBudget(total, "expense", "PDF OCR Total");
      rewardForScanning(); // ✅ Earn points for scanning
    }
  } catch (err) {
...
// 💰 Sync to Budget
// =======================
import { rewardForScanning } from './reward.js';
...
function addTotalToBudget(amount, type, label) {
  ...
  // ✅ Reward for scanning receipts or PDFs
  if (label.includes("Receipt") || label.includes("PDF")) {
    rewardForScanning();
  }
...
import { rewardForGoalCompletion } from './reward.js';
...
function updateGoalsUI() {
  ...
  goals.forEach((goal, index) => {
    ...
    const savedAmount = Math.max(0, Math.min(goal.amount, balance));

    const goalDiv = document.createElement("div");
    goalDiv.innerHTML = `
      <p><strong>${goal.name}</strong> — Saved: $${savedAmount.toFixed(2)} / $${goal.amount.toFixed(2)}</p>
      <div class="progress-bar"><div class="progress-fill" style="width:${progressPercent}%;"></div></div>
    `;
    progressContainer.appendChild(goalDiv);

    // ✅ Reward if goal is completed and not yet rewarded
    if (!goal.completed && balance >= goal.amount) {
      rewardForGoalCompletion();
      goal.completed = true; // mark as rewarded
    }
  });
...
import { rewardForAIUsage } from './main.js';
